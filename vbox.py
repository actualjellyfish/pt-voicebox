...
4/3/16: Some of the documentation, especially in the header, might be
out-of-date. Please let me know if you find something glaring!
...
you initialize a word by passing in a string
    ...
    sig: the significance score of the word, which is how much the frequency in
    this dictionary exceeds baseline frequency
    ...
    I introduced this to avoid a problem with large dictionaries: the top
    suggestions tend to be extremely common words, even given specific
    contexts. If the previous two words are 'jack the' then one continuation
    you'd like to have is 'ripper.' but if you go strictly by rate of
    occurrence, then phrases like 'jack, the thing' [is] and [i gave]
    'jack the keys' are going to overwhelm 'jack the ripper'

    To solve this, significance score tries to index how much the context in
    question boosts a word's frequency over baseline. This may be what you
    would call a Bayesian approach:

    sig = P(w|context)/P(w)

    So if 'ripper' occurs 1 percent of the time after 'jack the' but only
    .0000001 percent of the time overall, it gets a huge sigscore. meanwhile,
    'thing' might occur 2 percent of the time after 'jack the' but .05 percent
    of the time overall, giving it a more modest sigscore

    To avoid giving huge sigscores to sequences that occur just once or twice,
    I've mulitplied the sigscore by the log of the frequency as a way of
    discounting those little blips we should probably make the base of that
    log function into a parameter so we can adjust how much the program
    privileges sequences with high frequencies.

Voice: represents data from a given corpus. initialize it by passing in a
dictionary
    ...
    activeD: a pruned version of D containing only words that occur a certain
    number of times

    VoiceSettings: determines the pruning criterion for this voice and the way
    in which its words get ranked (i.e., by normalized frequency, raw frequency
    or significance score)

    methods:

    restrict(dictionary,list): given a list of words, takes a dictionary and
    returns a dictionary with only matching words

    atLeast(dictionary,int): finds all the words in a dictionary that pass a
    frequency criterion, passes this list to restrict()
...
Voicebox: a collection of Voice objects and their associated weights
    ...
    addTranscript: given a transcript where speakers are assigned to lines
    by SPEAKER: [line], breaks that script down into constituent speakers and
    adds a Voice for each
...
Context: constructed using a dictionary and a list of words (the context in
which we're typing), this represents all the influences on the what list of
words the program will suggest
...
# first one is weight to baseline dict, then to 1-back, then 2-back,
# then conjunction
default_wts = [0, 0, 1]
default_VSets = {
    'criterion': 3,
...
# contains information about a source stored as a dictionary of all
# distinct words
class Voice(object):
    def __init__(self, name, dict, number=1,
                 criterion=1, settings=default_VSets):
        self.criterion = criterion
        self.vision = settings['vision']
        self.ranktype = settings['ranktype']
        self.weights = settings['weights']
        # defines how much this voice privileges larger ngrams
        self.loveOfSize = settings['loveOfSize']
    ...
    def atLeast(self, D, name, n):
        print(
            'restricting {0} to words that occur at least {1} time(s)'.format(
                name, self.criterion))
        commonWords = []
        for word in self.D:
    ...
    # expresses the summed scores in an aggregate dictionary as a proportion
    # of the total
    ...
    # prints the whole dictionary. Is passed a string of several spaces as an
    # indent
    ...
    # returns a dictionary of words with the weights consolidated from all
    # ngrams derived from recent_words, plus baseline
    def weighContexts(self, D, recent_words, ranktype=''):
        ...
        toReturn = {}
        # choose the appropriate scoring metric (could add more scoring
        # metrics here)
        for w in normed:
            if ranktype == 'norm':
...
class Context(object):
    default_context_settings = {
        'baseline_wt': .00000001,
        'ngram_factor': 2,
        'twoback_wt': 1,
    }

    def __init__(self, D, prev_words, next_words,
                 weights=default_context_settings):
        self.baseline = D
    ...
    def set_ngram_wts(self, weights):
        ...
        for n in range(len(self.prev_ngrams)):
            ngram_wts.append(math.pow(
                weights['ngram_factor'], len(self.prev_ngrams)-n-1))
        return ngram_wts
    ...
    # TODO: implement these, which will enable backwards prediction
    # (which words precede others)
    ...
    # given a list of recent words in the sentence and a dictionary, returns
    # a list of dictionaries of words that follow after all sizes of ngrams
    ...
    # if the given sequence of words has occurred in the source,
    # returns the dictionary of words that followed. otherwise returns false
...
class Voicebox(object):
    ...
    # takes a transcript, breaks it up by speaker and
    # adds one voice for each speaker
    def addTranscript(self, tname, speakerlist):
        ...
        for i in range(1, len(lines)):
            labeled_lines.append(
                lines[i-1].split()[-1], lines[i].split()[0:-1])
        ...
        for line in labeled_lines:
            ...
            if name in speakerlist:
                ...
                # if the speakerlist has 'other' in it, then it consolidates 
                # all unclaimed lines into one
    ...
    # passed a string, makes a dictionary from a string.
    # passed an empty string and a file, makes it from a file
    def makeDict(self, str, filename=''):
        ...
        for s in range(0, len(sentences)):
            ...
            try:
                sentences[s] = sentences[s].translate(
        ...
        D = {}
        # go through each sentence, add each word to the dictionary,
        # incrementing length each time
        for s in sentences:
    ...
    # gives entries (and subentries) in D scores based on ratio to the
    # baseline of SuperD
    ...
    # Add word at position pos in sentence s to dictionary D with vision v.
    # Returns a dictionary.
    def addWord(self, D, s, pos, v):
        # extract string of word from the sentence
        word = s[pos]
        # base case. v is 0, so we're just returning D with the frequency of
        # s[w] incremented, not altering subdictionaries
        if v <= 0:
        ...
        # if v is greater than 0, we increment the frequency and
        # ask for subdictionaries
        elif v > 0:
            if word in D:
                newsub1 = D[word].sub1
                if pos < len(s)-1:
    ...
    def getVoices(self):
        ...
        for voicename in sorted(self.voices):
            print(voicename, ' '*(tab-len(voicename))+"(wt. "+str(
                self.voiceWeights[voicename])+")")

    # this takes a list of the most recent n words and returns a ranked list
    # of options for the next word
    def getOptions(self, recent_words):
        ...
        for v in self.voices:
            ...
            if self.voiceWeights != 0:
                ...
                for w in contribution:
                    if w in aggregate:
                        aggregate[w][0] += (
                            contribution[w] * self.voiceWeights[v])
                        aggregate[w][1][v] = (
                            contribution[w] * self.voiceWeights[v])
                    else:
                        aggregate[w] = [0, {}]
                    ...
                    if self.proportional_src:
                        for v in aggregate[w][1]:
                            aggregate[w][1][v] = aggregate[w][1][v] / sum(
                                aggregate[w][1].values())
        ...
        # returns as much of the top of the list as specified by num_opts
        optionList = list(reversed(ranked))[0:self.num_opts]
    ...
    def write(self, vb=voicebox):
        ...
        while 1:
            ...
            if response.isdigit():
                ...
                print("Invalid input. Choose a number between 1 and " + str(
                    vb.num_opts) + " or enter a word manually.")
                self.printLog(scriptlog+linelog, cur)
